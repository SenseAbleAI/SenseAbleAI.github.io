...
const UserAccountForm: React.FC<UserAccountFormProps> = ({ isLoginMode = false }) => {
  ...
  // In the live webapp, each visitor should start with a blank profile to fill in.
  // Clear any stale persisted profile so the form is empty and submitting registers anew.
  useEffect(() => {
    if (!IS_DEMO_MODE && isLoginMode) {
      setUser(null);
      setPreferences(null);
    }
  ...
  useEffect(() => {
    // Do not pre-fill the login form in the live webapp — keep it empty for the user.
    if (!IS_DEMO_MODE && isLoginMode) return;
    if (user) {
      setFormData(prev => ({
        ...prev,
        name: user.name,
